...
from . import torch_utils  # ┬átorch_utils, google_utils
...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    def metric(k):  # compute metric
        ...
        return (best > 1. / thr).float().mean()  # ┬ábest possible recall
...
def build_targets(p, targets, model):
    ...
    for i in range(det.nl):
        ...
        gain[2:] = torch.tensor(p[i].shape, device=device)[[3, 2, 3, 2]]
    ...
    for file in tqdm(sorted(glob.glob('%s/*.*' % path))):
        ...
        if any(i):
            ...
            with open('new/images.txt', 'a') as f:  # add image to dataset list
                f.write(img_file + '\n')
            with open('new/labels/' + Path(file).name, 'a') as f:  # write labels
                for l in labels[i]:
...
def print_mutation(hyp, results, bucket=''):
    # Print mutation results to evolve.txt (for use with images.py --evolve)
...
def output_to_target(output, width, height):
    ...
    return np.array(torch.tensor(targets).cpu())
...
def plot_results_overlay(start=0, stop=0):  # from utils.utils import *; plot_results_overlay()
    # Plot training 'results*.txt', overlaying images and val losses
    s = ['images', 'images', 'images', 'Precision', 'mAP@0.5', 'val', 'val', 'val', 'Recall', 'mAP@0.5:0.95']  # legends
    ...
    for fi, f in enumerate(files):
        try:
            ...
            for i in range(10):
                ...
                ax[i].set_title(s[i])
                # if i in [5, 6, 7]:  # share images and val loss y axes
