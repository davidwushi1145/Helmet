...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    @staticmethod
    def collate_fn(batch):
        imgs, label, path, shapes = zip(*batch)
        ...
        padded_imgs = []
        for img in imgs:
            padding_h = max_h - img.size(1)
            padding_w = max_w - img.size(2)
            padded_img = F.pad(img, (0, padding_w, 0, padding_h))
            padded_imgs.append(padded_img)

        for i, l in enumerate(label):
            l[:, 0] = i

        return torch.stack(padded_imgs, 0), torch.cat(label, 0), path, shapes
